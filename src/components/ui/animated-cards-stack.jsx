...
export const ContainerScroll = ({
  ...
  // "start start" syncs progress 0 with the moment the sticky child pins
  // (container top hits viewport top). The original "start center" burned
  // ~22% of progress while the stack was still scrolling normally, so the
  // first card(s) flew off before the pin engaged.
...
export const CardTransformed = React.forwardRef(
  (
    ...
    // NOTE: hooks must run unconditionally — the original TSX called
    // useMotionTemplate inside a ternary, which violates the rules of hooks
    // (it only worked because `variant` never changes at runtime).
